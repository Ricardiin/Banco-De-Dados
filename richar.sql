...
FROM estudantes
WHERE nascimento BETWEEN '2000-01-01' AND '2013-12-31' order by nascimento desc;
...
from estudantes
where profissao = 'contador' and sexo = 'M';

/*  4 */
select nome, nascimento,Nacionalidade,sexo
from estudantes
where sexo = 'F' and nome Like 'g%';
...
/* 6 */
select nome,nacionalidade,sexo,peso
from estudantes
where nome like '%Silva' and sexo = 'M' ;
...
/* 8 */
SELECT AVG(peso) AS media_peso_mulheres
FROM estudantes
WHERE sexo = 'F';
...
WHERE sexo = 'F'
AND nacionalidade <> 'Brasileira'
AND nascimento BETWEEN '1970-01-01' AND '2000-12-31';

/* 10 */
select sexo,count(*)
from estudantes
where altura > '1.60' and sexo = 'F';
...
INSERT INTO estudantes
values
(default,'Getulio Vargas','2004-03-15','M',88,1.65,'Brasileiro','Analista de Dados');
...
from estudantes
where nacionalidade <> 'Bras' 
group by nacionalidade
HAVING COUNT(*) > 2;

/* 9 */
SELECT nome, altura, sexo,peso, nacionalidade,nascimento
FROM estudantes
WHERE nascimento BETWEEN '1970-01-01' AND '2000-12-31'
and sexo = 'F'
AND altura < 1.65
AND nacionalidade NOT IN ('brasileira')
ORDER BY altura ASC;
...
from estudantes
where profissao like  'Adm%' 
or profissao like 'Progra%' 
or profissao like 'Vende%' ;

/* 12 */
select nome,sexo,nascimento
from estudantes
where nascimento between '2000-01-01' and '2010-01-01'
order by nascimento;

/* A */
select nome,nascimento,sexo
from estudantes
where nascimento >= '2000.01.01'
order by nascimento;
...
select nome,nacionalidade,count(*)
from estudantes
where nacionalidade like 'Arg%' 'Can%' 
group by nacionalidade;
...
from estudantes
where peso > '80'
and altura > (select avg(altura) from estudantes)
group by(altura);
...
add column cprefere int;

alter table estudantes
add foreign key(cprefere)
references cursos(idcurso);

update estudantes set cprefere = '1'
where id ='19';
...
/* Irá deletar o curso q não tenha nenhum aluno cadastrado 
delete from cursos
where idcurso = '2';*/

select nome, cprefere from estudantes;

select nome, ano from cursos;

select nome, cprefere estudantes;

/* */
select estudantes.nome, estudantes.cprefere,
cursos.nome, cursos.ano
from estudantes inner JOIN cursos
ON idcurso = estudantes.cprefere;
...
/* abreviando */
select e.nome, e.cprefere, c.nome, c.ano
from estudantes as e inner join cursos as c
on c.idcurso = e.cprefere
order by e.nome;

/* Tabela Esquerda*/
select e.nome, e.cprefere, c.nome, c.ano
from estudantes as e left join cursos as c
on c.idcurso = e.cprefere
order by e.nome;
